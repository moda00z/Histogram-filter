...
//	vector< vector <float> > newGrid;
	...
	int height = grid.size();
  	...
  	vector< vector <float> > newGrid(height, vector<float>(width, belief_per_cell)); // this is so much cleaner that whats commented down (feels dump looping for constant valuse) was suggested by reviewer; initialize the vector of vector in the constructor.
	...
	float p_miss) 
{
    ...
    //  bool hit = (color == grid[i][j]);
       float p = (color == grid[i][j])?p_hit:p_miss; // this is a great way to simplfiy the operation. was suggested by the reviewer.
       newGrid[i][j] = beliefs[i][j] * p;
